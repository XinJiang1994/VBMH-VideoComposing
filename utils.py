...
def composite4exec(frame1, frame2, pha1, pha2, params):
    # print(f'frame1.shape:{frame1.shape}, frame2.shape:{frame2.shape}')
    h, w = frame1.shape[:2]
    # frame2 = cv2.resize(frame2, (w, h), None)
    ...
    new_h, new_w = int(org_h2*zoom_rate), int(org_w2*zoom_rate)
    frame2 = cv2.resize(frame2, (new_w, new_h), None)
    pha2 = cv2.resize(pha2, (new_w, new_h), None)
    ...
    bg = np.zeros_like(frame1)
    bg[idx_x1_1:idx_x1_2, idx_y1_1:idx_y1_2] = frame2[idx_x2_1:idx_x2_2, idx_y2_1:idx_y2_2]
    com = np.uint8(frame1*pha1+bg*(1-pha1))

    bg_pha = np.zeros_like(pha1)
    bg_pha[idx_x1_1:idx_x1_2,
           idx_y1_1:idx_y1_2] = pha2[idx_x2_1:idx_x2_2, idx_y2_1:idx_y2_2]
    com_pha = np.maximum(pha1, bg_pha)
    return com, com_pha
...
#     '''
#     frame_target_new = frame_target*pha_target
#     frame_distract_new = frame_distract * pha_distract
